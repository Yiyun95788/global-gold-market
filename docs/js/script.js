...
// Viz 5 - Interactive Gold-Silver Regime Quilt
function createViz5() {
    const wrap = d3.select('#viz5 .viz-container');
    wrap.html('');

    // left: SVG + Right: Legend
    ...
    const years = d3.range(2000, 2026); // 2000–2025
    ...
    const btnSilver = btnBar.append('button')
        .text('Silver')
        .attr('class','asset-btn active');
    ...
    d3.csv(csvPath).then(raw => {
        ...
        const sel = cells.selectAll('rect.qcell')
            .data(grid)
            .enter().append('rect')
            .attr('class','qcell')
            .attr('rx', 7).attr('ry', 7)
        ...
        btnSilver.on('click', ()=>{
            btnSilver.classList.add('active');
